...
EOF
  ...
  echo "[.]:[...]"
  sleep .3
  echo "[#]:[d3ckn3t]:[AP]:[ping]:[$new_target]"
  sleep .3
  echo "[.]:[...]"
  if ping -c 1 "$new_target" &> /dev/null; then
    ssh_connect_alt
  else
    show_menu
  fi
...
# Function to connect to the target using alts SSH
ssh_connect_alt() {
  echo "[+]:[d3ckn3t]:[client]:[$new_target]:[Found]"
  sleep .2
  echo "[.]:[...]"
  sleep .2 
  echo "[+]:[d3ckn3t]:[target.lock]:[$new_user@$new_target]"
  sleep .2
  echo "[.]:[...]"
  ssh -p "$PORT" "$new_user@$new_target" #"$COMMAND"
}
