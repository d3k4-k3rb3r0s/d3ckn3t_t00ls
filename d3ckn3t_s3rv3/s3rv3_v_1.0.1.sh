...
|[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]|
    ...
    | _|   __| |___ /  ___| | __ _ __ |___ /| |_           ___|___ / _ ____   _|___ /  |_ |     [d3ckn3t-s3rv3]:[v]:[1.0.1]
    | |   / _` | |_ \ / __| |/ /| '_ \  |_ \| __|  _____  / __| |_ \| '__\ \ / / |_ \   | |     [d3k@t3ss3r4]:[2025]
...
echo "[↑]:[d3ckn3t-s3rv3]:[$hostname]:[$directory]:[server]:[$ip_address]:[$port]:[↑]"
echo "[↑]:[d3ckn3t-s3rv3]:[↑]:[↑]:[↑]:[↑]:[↑]:[][↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]:[↑]"
# Create a temporary Python script for the dark-themed server
echo "import http.server" > /tmp/dark_index.py
echo "import socketserver" >> /tmp/dark_index.py
echo "import os" >> /tmp/dark_index.py
echo "" >> /tmp/dark_index.py
echo "PORT = $port" >> /tmp/dark_index.py
echo "" >> /tmp/dark_index.py
cat << 'EOF' >> /tmp/dark_index.py
class DarkThemeHandler(http.server.SimpleHTTPRequestHandler):
    def list_directory(self, path):
        try:
            list_dir = os.listdir(path)
            list_dir.sort(key=str.lower)
            r = ['<!DOCTYPE html>',
                 '<html><head><title>Directory</title>',
                 '<style>body{background:#1a1a1a;color:#D2B48C;font-family:Arial}a{color:#90EE90;font-size:2.2em}</style></head><body>',
                 '<h1>Directory: ' + self.path + '</h1><ul>']
            for name in list_dir:
                fullname = os.path.join(path, name)
                displayname = name + ('/' if os.path.isdir(fullname) else '')
                r.append('<li><a href="' + displayname + '">' + displayname + '</a></li>')
            r += ['</ul></body></html>']
            ...
            self.send_error(404, "Error: " + str(e))

with socketserver.TCPServer(('', PORT), DarkThemeHandler) as httpd:
    httpd.serve_forever()
EOF

# Run the temporary script
python3 /tmp/dark_index.py

# Clean up the temporary file on exit
trap 'rm -f /tmp/dark_index.py' EXIT
